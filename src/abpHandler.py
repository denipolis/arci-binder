...
class ABPHandler(metaclass=Singleton):
  ...
  def loadProfileByPath(self, path) -> bool:
    ...
    if not parsedProfile['name']:
      QMessageBox(QMessageBox.Icon.Critical, "Импорт профиля", "Найдена ошибка в данных профиля. К сожалению, его невозможно загрузить.\nОшибка: не найдено секции name", QMessageBox.StandardButton.Ok).exec()
      return False
    ...
    if(self.database.isProfileExistsByHotkey(parsedProfile['hotkey'])):
      QMessageBox(QMessageBox.Icon.Critical, "Импорт профиля", f"Горячую клавишу \"{parsedProfile['hotkey']}\" уже использует профиль \"{self.database.findProfileByHotkey(parsedProfile['hotkey'])[0]}.\nУдалите его или поменяйте горячую клавишу, что бы загрузить новый профиль.", QMessageBox.StandardButton.Ok).exec()
      return False

    if(self.database.isProfileExistsByHotkey(parsedProfile['name'])):
      QMessageBox(QMessageBox.Icon.Critical, "Импорт профиля", f"Название \"{parsedProfile['hotkey']}\" уже используется.\nИзмените название прошлому профилю, что бы загрузить новый.", QMessageBox.StandardButton.Ok).exec()
      return False
    ...
    QMessageBox(QMessageBox.Icon.Information, "Импорт профиля", f"Профиль {parsedProfile['name']} успешно загружен!", QMessageBox.StandardButton.Ok).exec()
    return True